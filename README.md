...
面向主观交易者的 **价格行为（Price Action）** AI 辅助决策工具。从 **MT5 / TradingView / AkShare / 东方财富 / Tushare / 腾讯直连 / 通达信** 等数据源读取 K 线，将结构化 K 线数据与预计算特征送入大模型做**两阶段分析**（市场诊断 → 交易决策），**不是**截图识图，**不连接券商、不执行下单**。

---

## 主要功能

- 📈 **多数据源**：MT5（Windows）、TradingView（全平台）、AkShare / 东方财富 / Tushare（A 股）、腾讯直连、通达信（mootdx）、yfinance（期货/加密货币）
- 📊 **多周期支持**：1h / 4h / 日线 / 周线(1w) / 月线(1M)；K 线图可叠加 MA5 / MA10 / MA25 / MA60 均线（独立勾选显示，跨重启保留偏好）；A 股惯例涨红跌绿
- 🧠 **两阶段 AI 分析**：市场诊断 → 策略路由 → 交易决策（限价/突破/市价或不下单）
- 🔄 **增量分析与持续跟踪**：无新增 K 线时复用上次结论（跳过 AI 调用）；新增 K 线时增量分析；开启 `keep_analysis` 后新 K 线收盘自动触发新一轮分析；「重新分析」按钮可强制全量重跑
- 📄 **导出分析报告**：一键导出 Markdown 报告（含分析结论、依据、置信度、推演结论、盈亏比等）
- 🌳 **决策树可视化**：赛博科幻风格可交互流程图，自动播放闸门→策略路径动画
- 🔮 **未来走势预期**：AI 预测下一根 K 线方向和下一个市场周期位置
- 💬 **分析后自由追问**：完整对话会话管理器，实时推理流 + Token 进度条，对话历史持久化
- 🌐 **网络代理配置**：可单独为 AI 请求配置代理（http/socks5），行情数据源始终直连
...
| 项目     | 要求                                                                    |
| -------- | ----------------------------------------------------------------------- |
| 操作系统 | Windows 10 / 11（主支持）、macOS 12+（A 股数据源 / TradingView）        |
| Python   | 3.11+                                                                    |
