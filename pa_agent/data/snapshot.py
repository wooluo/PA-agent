...
# MA60 需要至少 60 根预热，留余量设 80（EMA20/ATR14 需求更小，80 足够覆盖）。
...
def compute_indicators(bars: list[KlineBar]) -> IndicatorBundle:
    """Compute EMA20, ATR14, and MA5/10/25/60 for *bars* (newest-first order).
    ...
    ma5_asc   = sma_full(closes, 5)
    ma10_asc  = sma_full(closes, 10)
    ma25_asc  = sma_full(closes, 25)
    ma60_asc  = sma_full(closes, 60)

    # Reverse back to newest-first
    return IndicatorBundle(
        ema20=tuple(reversed(ema20_asc)),
        atr14=tuple(reversed(atr14_asc)),
        ma5=tuple(reversed(ma5_asc)),
        ma10=tuple(reversed(ma10_asc)),
        ma25=tuple(reversed(ma25_asc)),
        ma60=tuple(reversed(ma60_asc)),
    )
...
def build_analysis_frame(
    ...
    *bars_raw* is newest-first. If ``bars_raw[0].closed`` is False it is the
    ...
    indicators = IndicatorBundle(
        ...
        ma5=indicators_all.ma5[:n],
        ma10=indicators_all.ma10[:n],
        ma25=indicators_all.ma25[:n],
        ma60=indicators_all.ma60[:n],
    )
    return KlineFrame(
        symbol=symbol,
        timeframe=timeframe,
